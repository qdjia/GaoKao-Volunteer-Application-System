...
本轮已完成 P0-1 数据库迁移骨架：默认数据源改为 PostgreSQL，加入 Flyway V1、Docker Compose 开发数据库和 Testcontainers 集成测试，并删除 H2 依赖及兼容 SQL。前端页面、业务领域模型和桌面启动脚本尚未改造。
...
| 数据库 | PostgreSQL + Flyway V1 已完成 | 随领域模型持续追加版本化迁移 |
...
然后从 **P0-2 领域模型重建** 开始。除非用户改变范围，不再恢复 H2，也不再继续完善旧专业录取、智能推荐、兴趣课程或自助注册。

## 十二、重要提醒

- 本文中 P0-1 已实现，其余阶段仍是已确认但尚未全部实现的需求。
