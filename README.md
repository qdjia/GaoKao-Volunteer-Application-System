...
> **当前阶段：P1-3.4 已完成，下一模块为 P1-3.5。**
> Compose 可按需建立只开放考生能力的 Quick Tunnel，并已提供独立的桌面启动与停止入口；停止前自动备份仍待完成。
>
...
| 在线统计 | 每 10 秒心跳、30 秒超时，包含首次改密页面 | 不延长登录有效期；Stop App 停止前显示人数 |
...
桌面会生成 `Gaokao - Start` 和 `Gaokao - Stop`。Start App 会检查并启动 Docker Desktop、等待 Compose 健康、建立 Quick Tunnel、显示考生公网地址并自动打开本机管理后台。关闭启动提示窗口或浏览器不会停止系统。

Stop App 要求输入管理员账号密码并查询实时在线人数；有人在线时必须连续两次输入 `STOP`，无人在线时确认一次。确认后先关闭公网隧道，再正常停止项目容器，并默认退出 Docker Desktop。停止脚本登录会遵循单设备会话规则；若查询后取消停止，浏览器管理员需要重新登录。

项目不会创建开机自启动任务，相关遗留安装脚本已删除。`start-app-session.ps1` 仅作为兼容入口转发到新版 Start App，不再监听浏览器关闭事件。

**当前备份边界：** P1-3.5 尚未完成，Stop App 只保留 PostgreSQL 持久化目录，不会生成独立备份文件；正式数据上线前不要把正常持久化等同于备份。
...
| Stop App | 管理员验证 → 检查在线人数 → 必要时二次确认 → 停止隧道与项目 → 退出 Docker |

在线统计采用每 10 秒心跳、30 秒超时；Stop App 通过本机管理员认证调用 `GET /api/admin/workflow/online`。关闭浏览器不会触发全局停止。数据库备份与30份轮转将在下一模块接入 Stop App。
...
| **P1-3** | **应用 Docker 化、公网隔离、Quick Tunnel、启停与备份** | **进行中（1-4 已完成）** |

下一模块实现停止前 PostgreSQL 一致性备份、最近30份轮转和备份失败保护；之后验证10人并发、重启恢复和断网恢复。旧原型表与服务需在备份、依赖核对后另行清理，不能直接删除已有数据。
